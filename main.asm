...
.text
	...
	invalidCpf:
		add $s4, $t0, -9			# set $s4 to first digit of current cpf
		add $s5, $s2, 1				# set $s4 to last digit of current cpf
		jal printCurrentCpf			# print current cpf
		la $a0, invalidMessage			# load invalid message
		jal printMessage			# print invalid message
		j jumpToNextCpf
	...
	printValidMessage:
		add $s4, $t0, -1			# set $s4 to first digit of current cpf
		add $s5, $s2, -1			# set $s4 to last digit of current cpf
		jal printCurrentCpf			# print current cpf
		la $a0, validMessage			# load valid message
		jal printMessage			# print valid message
		
		j jumpToNextCpf
	
	printCurrentCpf:
		lb $a0 fileWords($s4)			# load cpf digit in $a0
		add $a0, $a0, -48			# char to int conversion
		li $v0, 1				# print integer
		syscall
		add $s4, $s4, 1				# $s4 = $s4 + 1
		ble $s4, $s5, printCurrentCpf
		jr $ra
	...
	resetRegisters:
		li $t1, 0				# reset read number
		li $t3, 0				# reset multiplication result
		li $t4, 0				# reset multiplication accumulator
		jr $ra
